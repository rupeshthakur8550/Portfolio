...
const ProfileCard = ({ profile }: { profile: IntroProfile }) => {
  ...
  return (
    ...
    >
      <div className="absolute -inset-1 bg-gradient-to-r from-theme-sky via-theme-purple to-theme-pink rounded-2xl blur opacity-25 group-hover:opacity-75 transition duration-1000 group-hover:duration-200"></div>
      <div className="bg-theme-text/90 p-5 rounded-2xl border border-theme-text/10 shadow-lg overflow-hidden backdrop-blur-md text-theme-bg relative transform transition-all duration-500 hover:scale-[1.05] hover:-translate-y-2 hover:shadow-2xl mx-6 sm:mx-0 font-mono text-xs md:text-sm">
        {/* Mac Window Header */}
        <div className="flex gap-2 mb-3 border-b border-border/30 pb-3 relative z-20">
        ...
        {/* Dotted lines */}
        <div className="absolute top-0 right-6 md:right-8 w-0.5 h-16 md:h-20 border-l-2 border-dashed border-theme-sky/30 z-0"></div>
        <div className="absolute top-8 md:top-10 right-0 w-8 md:w-12 h-0.5 border-t-2 border-dashed border-theme-sky/30 z-0"></div>
        <div className="absolute bottom-0 left-6 md:left-8 w-0.5 h-12 md:h-40 border-l-2 border-dashed border-theme-sky/30 z-0"></div>
        <div className="absolute bottom-8 md:bottom-10 left-0 w-10 md:w-16 h-0.5 border-t-2 border-dashed border-theme-sky/30 z-0"></div>
        ...
        <div className="relative z-10 pt-2">
          ...
          <h2 className="text-xl md:text-2xl font-bold text-center mb-3 text-theme-bg">
            {profile.name}
          </h2>

          <div className="flex justify-center mb-4">
            {/* <div className="flex items-center gap-2 px-3 py-1 bg-theme-bg/10 rounded-full border border-theme-bg/20 backdrop-blur-sm shadow-sm">
 <span className="relative flex h-2 w-2">
 <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-theme-sky opacity-75"></span>
 <span className="relative inline-flex rounded-full h-2 w-2 bg-green-sky"></span>
 </span>
 <span className="text-[10px] sm:text-xs font-semibold text-theme-bg/90 tracking-wide uppercase">Currently @ Gruve.ai</span>
 </div> */}
          </div>
...
const IntroPage = () => {
  ...
  return (
    <div id="about">
      ...
      {/* Shared Stats Block */}
      <motion.div
        className="relative grid grid-cols-1 sm:grid-cols-3 gap-6 w-full max-w-5xl mx-auto sm:mt-10 md:mb-20 px-4"
        variants={{
          hidden: { opacity: 0 },
      ...
      >
        {introData.stats.map((stat, i) => (
          <motion.div
            ...
            className="bg-theme-text/90 backdrop-blur-md border border-theme-bg/10 rounded-xl p-6 text-center shadow-lg hover:shadow-xl hover:bg-theme-text transition-colors duration-300"
          >
